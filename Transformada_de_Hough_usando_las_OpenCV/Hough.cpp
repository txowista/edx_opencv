...
int main(int argc, char** argv)
{
	...
	Mat dst, cdst;
	Canny(src, dst, 50, 200, 3); //Detecci�n de los bordes en la imagen
	cvtColor(dst, cdst, CV_GRAY2BGR);

	vector<Vec2f> lines; // Vector para almacenar las lineas detectadas
	
	HoughLines(dst, lines, 1, CV_PI / 180, 200, 0, 0); // Detecci�n de las lineas en la imagen 
...
int main(int argc, char** argv)
{
	...
	/// Conversi�n de la imagen a niveles de gris
	cvtColor(src, src_gray, CV_BGR2GRAY);

	/// Reducci�n del ruido para evitar falsas detecciones
	GaussianBlur(src_gray, src_gray, Size(9, 9), 2, 2);

	vector<Vec3f> circles;

	/// Aplicaci�n de la Transformada de Hough para localizar los circulos
